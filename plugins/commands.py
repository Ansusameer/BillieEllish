...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    InlineKeyboardButton('⤬ Oᴜʀ Mᴀɪɴ Gʀᴏᴜᴘ Jᴏɪɴ ⤬', url=f'https://t.me//MOVIES_PROVIDE')
                ],[
                    ...
                    InlineKeyboardButton('🔻 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 🔻', url="https://t.me/MOVIES_PROVIDE2")
                  ]]
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEBqKtk09utv9oECruAkPhTGCEumfNkJAACNgkAAjDzgVZy2WNbJzXfJB4E") 
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MOVIES_PROVIDE ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@MOVIES_PROVIDE ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MOVIES_PROVIDE {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
                    InlineKeyboardButton("♻️Vᴇʀɪғʏ♻️", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                ],[
...
async def shortlink(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(f"<b>Hey {message.from_user.mention}, This command only works on groups !\n\n<u>Follow These Steps to Connect Shortener:</u>\n\n1. Add Me in Your Group with Full Admin Rights\n\n2. After Adding in Grp, Set your Shortener\n\nSend this command in your group\n\n—> /shortlink ""{your_shortener_website_name} {your_shortener_api}\n\n#Sample:-\n/shortlink greymatterslinks.in c762b2ab920b006a9bf7eed729589026d1da16b6\n\nThat's it!!! Enjoy Earning Money 💲\n\n[[[ Trusted Earning Site - https://greymatterslinks.in ]]]\n\nIf you have any Doubts, Feel Free to Ask me - @Mr_HKs\n\n(Puriyala na intha bot la message pannunga - @Mr_HKs)</b>")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
