...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('👻 ᴍᴏᴠɪᴇ ᴄʜᴀɴɴᴇʟ', url="https://t.me/MYFLiiX_2"),
                    InlineKeyboardButton('🫨 ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ', url="https://t.me/MYFLiiX_2")
                ],[
                    InlineKeyboardButton('🔻 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 🔻', url="https://t.me/MYFLiiX_2")
                  ]]
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('🍁 ᴜᴘᴅᴀᴛᴇ 🍁', url="https://t.me//MYFLiiX_2"),
                    InlineKeyboardButton('✨ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ ✨', url="https://t.me//MYFLiiX_2")
                ],[
            ...
            try:
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          InlineKeyboardButton('⸙ ʙᴏᴛ ᴏᴡɴᴇʀ ⸙', url=f'https://t.me/xayoonara'),
                          InlineKeyboardButton('💫 ᴍᴏᴠɪᴇ ᴄʜᴀɴɴᴇʟ 💫', url="https://t.me/MYFLiiX_2")
                       ],[
                          InlineKeyboardButton("🍁 ᴍᴏᴠɪᴇ ʀᴇϙᴜᴇꜱᴛ ɢʀᴏᴜᴘ 🍁", url="https://t.me/MYFLiiX_2")
                         ]
                        ]
                    )
                )
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MYFLiiX_2 ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@MYFLiiX_2 ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MYFLiiX_2 {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("♻️Vᴇʀɪғʏ♻️", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                ],[
                    InlineKeyboardButton("⁉️Hᴏᴡ Tᴏ Vᴇʀɪғʏ⁉️", url=f'https://t.me/MYFLiiX/774')
        ]]
        await message.reply_text(
            text="<b>You are not verified !\nKindly verify to continue !</b>",
            protect_content=True,
